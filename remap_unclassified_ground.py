...
Sector-boundary and outlier filtering deliberately do not belong here; that
responsibility is implemented by filter_laz_to_sector.py.
"""
...
def grid_layout(header: Any) -> tuple[float, float, int, int, int]:
    """Return a bounded 1 m grid derived from the LAS header."""
...
def analyse_file(path: Path) -> dict[str, Any]:
    """Measure the class-1 remap trigger without rewriting the source."""
    ...
    if class1_count:
        with laspy.open(path) as reader:
            for points in reader.chunk_iterator(CHUNK_SIZE):
                ...
                class1_x = x[is_class1]
                class1_y = y[is_class1]
                class1_z = z[is_class1]
                ids, valid = cell_ids(
                    class1_x,
                    class1_y,
                    min_x,
...
def remap_trigger(
    ...
    """Return whether the class-1 population is affected."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Detect/remap ground-like LAS class-1 points."
    )
    parser.add_argument(
        "--analyse-only",
        action="store_true",
